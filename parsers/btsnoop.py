...
class BTSnoopParser:
    ...
    def __init__(self, file_path: str, vendor_type_map: bool = False):
        self.file_path = Path(file_path)
        self.records: List[HCIRecord] = []
        self.header_info: Dict[str, Any] = {}
        self.base_timestamp = None
        self.vendor_type_map = vendor_type_map

    def _strip_hci_indicator(self, data: bytes, flags: int) -> Tuple[bytes, bool]:
        indicators = HCI_INDICATORS_VENDOR if self.vendor_type_map else HCI_INDICATORS
        if not data:
            return data, False
    ...
    def parse(self) -> List[HCIRecord]:
        ...
        if raw_data[:8] != b'btsnoop\x00':
            raise ValueError(f"无效的 BTSnoop 魔数: {raw_data[:8].hex()}")
        ...
        type_map = HCI_TYPES_VENDOR if self.vendor_type_map else HCI_TYPES
        offset = 16
        seq = 1
        ...
        while offset + 24 <= len(raw_data):
            ...
            flags = struct.unpack('>I', raw_data[offset + 8:offset + 12])[0]
            ...
            hci_type = flags & 0xFFFF
            packet_type = type_map.get(hci_type, f"Unknown({hci_type})")
            direction = "Host->Controller" if hci_type in (0, 1, 2) else "Controller->Host" if hci_type in (3, 4, 5) else "Unknown"
    ...
    def _parse_payload(self, record: HCIRecord) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.vendor_type_map:
                if record.flags == 2:
                    if len(data) >= 4 and data[0] == 0x01:
                        return parse_hci_command(data[1:])
                    return parse_hci_acl(data)
                if record.flags == 4:
                    return parse_hci_acl(data)
                return {'raw_hex': data.hex()[:64]}
            else:
                if record.flags == 2 and len(data) >= 4 and data[0] == 0x01:
                    return parse_hci_command(data[1:])
                if record.flags in (1, 4):
                    return parse_hci_acl(data)
                return {'raw_hex': data.hex()[:64]}
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        for rec in self.records:
            if rec.flags == 0:
                stats['commands'] += 1
            elif self.vendor_type_map and rec.flags == 2 and rec.parsed and rec.parsed.get('type') == 'HCI_Command':
                stats['commands'] += 1
            elif rec.flags == 3:
                stats['events'] += 1
                if rec.parsed:
            ...
            elif (rec.flags == 2 if self.vendor_type_map else rec.flags == 1):
                if self.vendor_type_map and rec.flags == 2 and rec.parsed and rec.parsed.get('type') == 'HCI_Command':
                    pass
                else:
                    stats['acl_tx'] += 1
    ...
    def _write_md_command_completes(self, f, stats: Dict[str, Any]) -> None:
        ...
        f.write("| 序号 | 时间(ms) | 命令 | OGF | OCF | 状态 |\n|------|----------|------|-----|-----|------|\n")
        for cc in stats['command_completes'][:10]:
            f.write(f"| {cc['seq']} | {cc['timestamp_ms']:.2f} | {cc['command']} | {cc['ogf']} | {cc['ocf']} | {cc['status']} |\n")
    ...
    def _write_md_connections(self, f, stats: Dict[str, Any]) -> None:
        if not stats['connections_established']:
            return
        f.write("## 🔗 连接建立事件\n\n")
        f.write("| 序号 | 时间(ms) | 句柄 | 设备地址 | 状态 |\n|------|----------|------|----------|------|\n")
        for c in stats['connections_established']:
            f.write(f"| {c['seq']} | {c['timestamp_ms']:.2f} | {c['handle']} | {c['bd_addr']} | {c['status']} |\n")
        f.write("\n")
    ...
    def _write_md_disconnections(self, f, stats: Dict[str, Any]) -> None:
        if not stats['connections_disconnected']:
            return
        f.write("## 🔴 断开连接事件\n\n")
        f.write("| 序号 | 时间(ms) | 句柄 | 原因 |\n|------|----------|------|------|\n")
        for d in stats['connections_disconnected']:
            f.write(f"| {d['seq']} | {d['timestamp_ms']:.2f} | {d['handle']} | {d['reason']} |\n")
        f.write("\n")
    ...
    def _write_md_errors(self, f, stats: Dict[str, Any]) -> None:
        if not stats['errors']:
            return
        f.write("## ⚠️ 错误事件\n\n")
        f.write("| 序号 | 时间(ms) | 事件 | 状态码 | 说明 |\n|------|----------|------|--------|------|\n")
        for e in stats['errors'][:20]:
            f.write(f"| {e['seq']} | {e['timestamp_ms']:.2f} | {e['event']} | {e['status_hex']} | {e['status_str']} |\n")
        f.write("\n")

    def _write_md_flow_table(self, f, records_to_export: List[HCIRecord]) -> None:
        f.write("## 📋 详细通信流程\n\n```\n")
        f.write(f"{'#':<5} {'Delta(ms)':<12} {'Direction':<18} {'Type':<12} {'OGF':<6} {'OCF':<6} {'Details'}\n")
        f.write("-" * 100 + "\n")
        ...
        for rec in records_to_export:
            ...
            f.write(f"{rec.seq:<5} {rec.timestamp_rel:<12.2f} {rec.direction:<18} {rec.packet_type:<12} {ogf_str:<6} {ocf_str:<6} {details}\n")
        f.write("```\n\n")
